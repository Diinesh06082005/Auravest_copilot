...
function LiveStockChart({ symbol, name, basePrice, baseChange }: LiveStockChartProps) {
  const [currentPrice, setCurrentPrice] = useState(basePrice);
  const [priceChange, setPriceChange] = useState(baseChange);
  const [history, setHistory] = useState<number[]>(() => {
    const hist = [];
    let p = basePrice * 0.95;
    const isUp = baseChange >= 0;
    for (let i = 0; i < 20; i++) {
      const bias = isUp ? 0.003 : -0.003;
      p = p * (1 + bias + (Math.random() * 0.01 - 0.005));
      hist.push(p);
    }
    return hist;
  });
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      const isUp = baseChange >= 0;
      const bias = isUp ? 0.001 : -0.001;
      const pct = bias + (Math.random() * 0.008 - 0.004);
      ...
      setPriceChange(prev => prev + (pct * 100));
      
      setHistory(prev => {
        const nextHist = [...prev.slice(1), newPrice];
        return nextHist;
      });

      const timeout = setTimeout(() => setTickDirection(null), 400);
  ...
  const points = history.map((val, idx) => {
    ...
    const y = svgHeight - ((val - min) / range) * (svgHeight - 16) - 8;
    return { x, y };
  });
  ...
  const fillGradientId = `grad-${symbol}`;

  return (
    <div className="bg-white/40 dark:bg-[#0f172a]/40 border border-slate-200/60 dark:border-slate-800/80 rounded-2xl p-4 flex items-center justify-between gap-4 transition-all duration-300 hover:border-blue-500/30">
      <div className="flex flex-col justify-between h-full min-w-[120px]">
        <div>
          <span className="text-xs font-black tracking-wider text-slate-400 dark:text-slate-500 block uppercase">
            {symbol}
          </span>
          <span className="text-sm font-bold text-slate-800 dark:text-white truncate block max-w-[130px]">
            {name}
          </span>
        </div>
        <div className="mt-3">
          <span className={`text-base font-black transition-colors duration-250 ${
            tickDirection === 'up' ? 'text-emerald-500 font-extrabold' : 
            tickDirection === 'down' ? 'text-rose-500 font-extrabold' : 
      ...
      <div className="relative flex-1 h-[70px] max-w-[240px]">
        <svg width="100%" height="100%" viewBox={`0 0 ${svgWidth} ${svgHeight}`} className="overflow-visible">
          ...
          <line x1="0" y1={svgHeight * 0.25} x2={svgWidth} y2={svgHeight * 0.25} stroke="currentColor" className="text-slate-105 dark:text-slate-900" strokeWidth="1" strokeDasharray="4 4" />
          <line x1="0" y1={svgHeight * 0.5} x2={svgWidth} y2={svgHeight * 0.5} stroke="currentColor" className="text-slate-105 dark:text-slate-900" strokeWidth="1" strokeDasharray="4 4" />
          <line x1="0" y1={svgHeight * 0.75} x2={svgWidth} y2={svgHeight * 0.75} stroke="currentColor" className="text-slate-105 dark:text-slate-900" strokeWidth="1" strokeDasharray="4 4" />
...
function InteractiveLoader({ ticker }: { ticker: string }) {
  ...
  useEffect(() => {
    const symbols = ['AAPL', 'TSLA', 'NVDA', 'MSFT', ticker.toUpperCase()];
    const interval = setInterval(() => {
      const sym = symbols[Math.floor(Math.random() * symbols.length)];
      const isBuy = Math.random() >= 0.45;
      const size = Math.floor(Math.random() * 450) + 10;
      
      const baseP = sym === 'AAPL' ? 185.50 : sym === 'TSLA' ? 177.40 : sym === 'NVDA' ? 875.12 : sym === 'MSFT' ? 421.90 : 150.00;
      const tradeP = baseP * (1 + (Math.random() * 0.02 - 0.01));
